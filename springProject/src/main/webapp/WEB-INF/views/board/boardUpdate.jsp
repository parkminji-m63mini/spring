...
<body>
			...
			<!-- enctype="multipart/form-data" -->
					...
					<div class="boardImg" id="titleImgArea">
						<img id="titleImg" width="200" height="200" 
							...
							<%-- titleImgSrc 변수가 만들어진 경우
								...
								-> img 태그의 src 속성을 추가하고
									속성 값으로 titleImgSrc 의 값을 작성하겠다.								
							 --%>
				...
				<div class="form-inline mb-2">
					...
					<div class="mr-2 boardImg" id="contentImgArea1">
						<img id="contentImg1" width="150" height="150"	<c:if test="${! empty contentImgSrc1}">src="${contentImgSrc1}"</c:if>>
						<button type="button" class="deleteImg">삭제</button>
					</div>

					<div class="mr-2 boardImg" id="contentImgArea2">
						<img id="contentImg2" width="150" height="150" <c:if test="${! empty contentImgSrc2}">src="${contentImgSrc2}"</c:if>>
						<button type="button" class="deleteImg">삭제</button>
					</div>

					<div class="mr-2 boardImg" id="contentImgArea3">
						<img id="contentImg3" width="150" height="150" <c:if test="${! empty contentImgSrc3}">src="${contentImgSrc3}"</c:if>>
						<button type="button" class="deleteImg">삭제</button>	
					</div>
				</div>
	...
	<script>
		...
		$(".deleteImg").on("click", function(event){
			...
			// 이미지 태그 삭제
			// 1. img 태그의 src 속성을 제거 (x)
			//$el.removeAttr("src"); -> 이미지 영역은 남지만 이지미가 엑스박스가 생겨서 올바르지 않다.
			...
			// 파일을 업로드했다가 삭제했을 때 값을 없애는 거 
			// input type="file" 태그에 있는 value값 초기화(삭제)
			...
			// 해야할 동작
			// 1) 서버에 삭제여부 알림
			// 현재 게시글에서 이미지목록 조회해서 / 그중에 파일 레벨을 삭제 -> 배열로 t,f 구분해서 확인
			
			// 2) 다시 올렸다가 이미지를 지웠을 때 히든에 있는 밸류 값 없애주기
		...
		function validate() {
			...
			for(var i=0; i<deleteImages.length ; i++){
				var $deleteImages = $("<input>", {type : "hidden", 
												name : "deleteImages",
