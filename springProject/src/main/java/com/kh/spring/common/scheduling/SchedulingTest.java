...
public class SchedulingTest {
	...
	// servlet-context.xml -> NameSpaces 탭 -> task 체크 후 저장
	// -> source 탭 이동 -> <task:annotation-driven/> 작성
	
	private Logger logger = LoggerFactory.getLogger(SchedulingTest.class);
	
	/* @Scheduled 속성
     *  - fixedDelay : 이전 작업이 끝난 시점으로 부터 고정된 시간(ms)을 설정. EX)메소드가 끝나면 지정된 시간만큼 세어라
     *  - fixedRate : 이전 작업이 수행되기 시작한 시점으로 부터 고정된 시간(ms)을 설정. EX)지정된 메소드가 시작됨과 동시에 카운트해라
     * 
     * * cron 속성 : UNIX계열 잡 스케쥴러 표현식으로 작성 
     * - cron="초 분 시 일 월 요일 [년도]" 
     * - 요일 : 1(SUN) ~ 7(SAT) 
     * ex) 2019년 9월 16일 월요일 10시 30분 20초 
     * 		cron="20 30 10 16 9 2(월요일임)" // 연도 생략 가능
     * 
     * - 특수문자 
     * * : 모든 수. 
	...
	//@Scheduled(fixedDelay =  10000) // 이전 동작 완료 후 10초 뒤에 다시 수행
			    // cron="초 분 시 일 월 요일 [년도]" 
	//@Scheduled(cron = "0 * * * * *") // 매 분이 시작될 떄 마다
	public void test() {
		logger.debug("-------------------------------스케쥴링 테스트 ------------------------");
		// boardService.deleteBoard(boardNo); // 주의사항 확인
	}
