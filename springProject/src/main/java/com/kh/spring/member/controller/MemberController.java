...
// + Model.getAttribute("loginMember") -> Session에서 "loginMember"라는 key를 가지는 속성의 값을 얻어와라
...
public class MemberController {
   ...
   */
      ...
      */
      ...
      // --> 커맨드 객체  (뭔가 자동으로 setter로 값을 넣어준 객체를 말하는듯함)
      /*@RequestMapping("loginAction")
...
//            member.setMemberPwd(memberPwd);
      ...
      public String updateAction(Member upMember, Model model, RedirectAttributes rdAttr, HttpServletRequest request) {
    	  										// 이거 작성한 이유? ***여기에      // 메세지 작성하고 싶을 때 적기
          ...
          if(result > 0) {
             ...
             // -> model.addAttirbute("loginMember", upMameber);
             // @SessionAttributes("loginMember")
    	  ...
    	  // select -> forward
          // select를 하는 경우 DB에서 조회한 데이터를 이용해 응답 화면을 만들어야함.
          // -> 응답화면을 쉽게 만들기 위해 사용하는게 JSP
          // -> JSP로 요청을 위임하기 위해서는 forward를 해야함.
          // 단, 로그인 같은 경우에는 조회 데이터를 세션에 올리는 작업을 해야하기 때문에 select 이지만 redirect 한다.
      ...
      public String deleteMember(String memberPwd, Model model, RedirectAttributes rdAttr, SessionStatus sessionStatus) {
            ...
            if(result > 0) {
               ...
               msg = "회원 탈퇴 되었습니다.";
               url = "/";
               sessionStatus.setComplete(); // 세션만료
            }else {
          	  status = "error";
          	  msg = "회원 탈퇴 실패";
          	  text = "현재 비밀번호를 확인해주세요.";
      ...
      /* 스프링에서 예외를 처리하는 방법
       * 1) try-catch, throws를 이용한 예외처리 방법
       * -> 메소드 레벨
       * 
